...
class BollingerBands(MultiMetricsBase):
    ...
    def run(self)-> list[tuple[str,float]]:
        if len(self.window) < self.N:
            return [ (self.key_upper, 0.0), (self.key_middle, 0.0), (self.key_lower,0.0) ]
        ...
        return [ (self.key_upper, upper), (self.key_middle, sma), (self.key_lower,lower) ]
...
class MACD(MultiMetricsBase):
    def __init__(self, key_macd:str, key_signal:str, key_histogram:str, short_N:int=12, long_N:int=26, signal_N:int=9):
    ...
    def run(self)-> list[tuple[str,float]]:
        if self.macd is None:
            return [ (self.key_macd,0.0), (self.key_signal,0.0), (self.key_histogram,0.0) ]
        _,signal_val = self.signal_ema.run()
        histogram = self.macd - signal_val
        return [ (self.key_macd,self.macd), (self.key_signal,signal_val), (self.key_histogram,histogram) ]
...
class ADX(SingleMetricsBase):
    ...
    def update(self, trend: Trend):
        ...
        if len(self.plus_dm) == self.N:
            ...
            dx = abs(plus_di - minus_di) / (plus_di + minus_di if (plus_di + minus_di) != 0 else 1)
            self.adx = 100 * dx
